...
func CollectSystemMetric(ctx context.Context) client.SystemMetric {
    ...
    if avg, err := load.AvgWithContext(ctx); err == nil { out.CPULoad = avg.Load1 }
    ...
    if p, err := process.NewProcess(int32(os.Getpid())); err == nil {
        if pm, err := p.MemoryInfoWithContext(ctx); err == nil && pm != nil {
            ...
            if out.ProcMaxMemory > 0 { out.ProcMemUsage = usedGB/out.ProcMaxMemory }
        }
    }
    score := 100.0
    if out.CPULoad > 0 { score -= out.CPULoad * 5 }
    if out.DiskUsageRatio > 0 { score -= out.DiskUsageRatio * 20 }
    if out.ProcMemUsage > 0 { score -= out.ProcMemUsage * 30 }
    if score < 0 { score = 0 }
    out.Score = score
    return out
}
