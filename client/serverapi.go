...
func (h *httpServerAPI) Acquire(ctx context.Context, base string, appID int64, currentServer, clientVersion string) (string, error) {
    ...
    if currentServer != "" { v.Set("currentServer", currentServer) }
    v.Set("protocol", "HTTP")
    if clientVersion != "" { v.Set("clientVersion", clientVersion) }
    u := fmt.Sprintf("http://%s/server/acquire?%s", base, v.Encode())
    var resp CommonResp[string]
    if err := h.get(ctx, u, &resp); err != nil { return "", err }
    if !resp.Success { return "", fmt.Errorf("acquire failed: %s", resp.Message) }
    return resp.Data, nil
}
...
func SafeLogErr(err error, msg string) {
    if err != nil { logging.L().Error(context.Background(), msg, "err", err) }
}
