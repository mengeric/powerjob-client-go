    ...
    go func() {
        ...
        for {
            ...
            case <-ticker.C:
                ...
                if err != nil { logging.L().Warn(ctx, "acquire server failed", "err", err); continue }
                if addr != "" { d.current.Store(addr) }
            }
        }
    }()
}
...
// Get 返回当前 server 地址。
func (d *Discovery) Get() string { v := d.current.Load(); if v==nil { return d.bootstrap }; return v.(string) }
