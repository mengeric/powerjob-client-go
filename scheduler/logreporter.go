    ...
    go func() {
        ...
        flush := func() {
            ...
            if err := l.api.ReportLog(ctx, l.disc.Get(), req); err != nil {
                logging.L().Warn(ctx, "report log failed", "count", len(buf), "err", err)
            }
            buf = buf[:0]
        }
        for {
            ...
            case <-ctx.Done():
                ...
                return
            case it := <-l.ch:
                if it.InstanceID == 0 || it.LogTime == 0 { continue }
                buf = append(buf, it)
                if len(buf) >= l.max { flush() }
            case <-ticker.C:
                flush()
            }
        }
...
func (l *LogReporter) Enqueue(it client.InstanceLogContent) {
    ...
    case l.ch <- it:
    default:
        logging.L().Warn(context.Background(), "log queue full, drop", "iid", it.InstanceID)
    }
}
